...
const Place = (props) => {
  ...
  const gatherLog = (id, time) => {
    ...
    setError("");
    axiosInstance
      .post("/view/log/outsider", { placeId: id, time })
      .then((res) => {
        console.log(res.data);
        setLogLoading(false);
  ...
  const getList = () => {
    let newData = data;
    newData = newData.filter((el) => el.type === "outsider");
        ...
        <div className="view-places-table">
          <div className="places-container">
            <div className="places-header d-flex justify-content-between flex-wrap">
              <div className="add-visitor-container flex-row fit-content">
                <div className={add ? "hide" : "reveal"}>
                  <AsyncButton
                    disabled={loading}
                    onClick={() => setAdd(true)}
                ...
                <div className={add ? "reveal" : "hide"}>
                  <AddPlace close={() => setAdd(false)} fetchData={fetchData} />
                </div>
              </div>
              <RefreshButton
                onClick={fetchData}
